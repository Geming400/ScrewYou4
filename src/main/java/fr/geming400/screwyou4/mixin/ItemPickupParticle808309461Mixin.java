...
public class ItemPickupParticle808309461Mixin {
    ...
    private void tick_846584200(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(846584200L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "getGroup()Lnet/minecraft/client/particle/ParticleRenderType;", cancellable = true)
    private void getGroup__203628748(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-203628748L))
