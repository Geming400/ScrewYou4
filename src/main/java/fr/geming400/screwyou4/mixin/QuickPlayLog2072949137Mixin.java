...
public class QuickPlayLog2072949137Mixin {
    ...
    private void log__1704998798(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(-1704998798L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "of(Ljava/lang/String;)Lnet/minecraft/client/quickplay/QuickPlayLog;", cancellable = true)
    private static void of__1675960353(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1675960353L))
    ...
    private void setWorldData_986152304(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(986152304L))
            info.cancel();
    }
