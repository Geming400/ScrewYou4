...
public class GoToWantedItem2010352381Mixin {
    ...
    private static void create__163753581(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-163753581L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "create(FZI)Lnet/minecraft/world/entity/ai/behavior/BehaviorControl;", cancellable = true)
    private static void create__1525863740(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1525863740L))
            info.setReturnValue(null);
    }
