...
public class SynchronizeRegistriesTask864674658Mixin {
    ...
    private void type_960555568(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(960555568L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "start(Ljava/util/function/Consumer;)V", cancellable = true)
    private void start__1846244447(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(-1846244447L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "handleResponse(Ljava/util/List;Ljava/util/function/Consumer;)V", cancellable = true)
    private void handleResponse_848357623(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(848357623L))
            info.cancel();
    }
