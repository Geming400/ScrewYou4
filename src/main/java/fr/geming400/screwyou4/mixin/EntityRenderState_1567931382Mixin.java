...
public class EntityRenderState_1567931382Mixin {
    ...
    private void fillCrashReportCategory__1029588368(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(-1029588368L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "appearsGlowing()Z", cancellable = true)
    private void appearsGlowing_741563445(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(741563445L))
