...
public class PublishCommand_1960947672Mixin {
    ...
    private static void register_1017387448(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(1017387448L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "getSuccessMessage(I)Lnet/minecraft/network/chat/MutableComponent;", cancellable = true)
    private static void getSuccessMessage__1734785709(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1734785709L))
