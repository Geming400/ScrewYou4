...
public class ChestSpecialRenderer_1237958197Mixin {
    ...
    private void submit__1005689186(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(-1005689186L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "getExtents(Ljava/util/function/Consumer;)V", cancellable = true)
    private void getExtents_898189348(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(898189348L))
            info.cancel();
    }
