...
public class WeatheringCopperChestBlock_1540619177Mixin {
    ...
    private void getAge__520326079(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-520326079L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "getAge()Lnet/minecraft/world/level/block/WeatheringCopper$WeatherState;", cancellable = true)
    private void getAge_2127803088(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(2127803088L))
            info.setReturnValue(net.minecraft.world.level.block.WeatheringCopper.WeatherState.EXPOSED);
    }

    @Inject(at = @At("HEAD"), method = "codec()Lcom/mojang/serialization/MapCodec;", cancellable = true)
    private void codec__1018494396(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1018494396L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "isWaxed()Z", cancellable = true)
    private void isWaxed_1578897759(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(1578897759L))
