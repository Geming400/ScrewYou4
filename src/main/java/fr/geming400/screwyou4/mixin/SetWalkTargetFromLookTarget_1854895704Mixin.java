...
public class SetWalkTargetFromLookTarget_1854895704Mixin {
    ...
    private static void create__1780395474(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1780395474L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "create(FI)Lnet/minecraft/world/entity/ai/behavior/OneShot;", cancellable = true)
    private static void create__731413429(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-731413429L))
            info.setReturnValue(null);
    }
