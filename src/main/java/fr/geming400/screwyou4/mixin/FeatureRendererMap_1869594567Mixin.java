...
public class FeatureRendererMap_1869594567Mixin {
    ...
    private void get__397651883(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-397651883L))
    ...
    private void put_1080779140(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(1080779140L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "values()Ljava/lang/Iterable;", cancellable = true)
    private void values__718389712(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-718389712L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "close()V", cancellable = true)
    private void close__244980863(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(-244980863L))
            info.cancel();
    }
    ...
    private void getOrThrow_269595724(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(269595724L))
            info.setReturnValue(null);
    }
