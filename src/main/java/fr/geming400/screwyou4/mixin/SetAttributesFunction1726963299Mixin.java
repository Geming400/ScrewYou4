...
public class SetAttributesFunction1726963299Mixin {
    ...
    private void run__1335922674(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1335922674L))
            info.setReturnValue(null);
    }

    @Inject(at = @At("HEAD"), method = "validate(Lnet/minecraft/world/level/storage/loot/ValidationContext;)V", cancellable = true)
    private void validate_708244528(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(708244528L))
            info.cancel();
    }
    ...
    private static void modifier__1074413171(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1074413171L))
            info.setReturnValue(null);
    }
    ...
    private static void setAttributes__1398315099(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1398315099L))
            info.setReturnValue(new net.minecraft.world.level.storage.loot.functions.SetAttributesFunction.Builder());
    }

    @Inject(at = @At("HEAD"), method = "codec()Lcom/mojang/serialization/MapCodec;", cancellable = true)
    private void codec__1501558743(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(-1501558743L))
            info.setReturnValue(null);
    }
