...
public class TheEndPortalRenderer_1300294501Mixin {
    ...
    private void submit_1096850986(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(1096850986L))
            info.cancel();
    }
    ...
    private void submit_882230195(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(882230195L))
            info.cancel();
    }
    ...
    private void createRenderState_539955140(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(539955140L))
            info.setReturnValue(new net.minecraft.client.renderer.blockentity.state.EndPortalRenderState());
    }

    @Inject(at = @At("HEAD"), method = "createRenderState()Lnet/minecraft/client/renderer/blockentity/state/BlockEntityRenderState;", cancellable = true)
    private void createRenderState_1683057325(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(1683057325L))
