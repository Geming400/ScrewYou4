...
public class ConduitSpecialRenderer921958574Mixin {
    ...
    private void submit_1360903728(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(1360903728L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "getExtents(Ljava/util/function/Consumer;)V", cancellable = true)
    private void getExtents_1602974987(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(1602974987L))
            info.cancel();
    }
