...
public class CastleSmallCorridorLeftTurnPiece323838804Mixin {
    ...
    private void postProcess__1896584139(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(-1896584139L))
            info.cancel();
    }
    ...
    private void addChildren_568702766(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(568702766L))
            info.cancel();
    }
    ...
    private static void createPiece_530164388(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(530164388L))
