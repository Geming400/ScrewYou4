...
public class VillagerPanicTrigger561190815Mixin {
    ...
    private static void isHurt_321162647(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(321162647L))
            info.setReturnValue(true);
    }

    @Inject(at = @At("HEAD"), method = "hasHostile(Lnet/minecraft/world/entity/LivingEntity;)Z", cancellable = true)
    private static void hasHostile_300367358(CallbackInfoReturnable<Object> info) {
        if (!ScrewYou4.isMethodAlive(300367358L))
            info.setReturnValue(true);
    }
