...
public class SkullSpecialRenderer_96307985Mixin {
    ...
    private void submit_535253138(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(535253138L))
            info.cancel();
    }

    @Inject(at = @At("HEAD"), method = "getExtents(Ljava/util/function/Consumer;)V", cancellable = true)
    private void getExtents_777324397(CallbackInfo info) {
        if (!ScrewYou4.isMethodAlive(777324397L))
            info.cancel();
    }
